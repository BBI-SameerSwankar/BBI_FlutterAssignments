...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  Future<void> _onSaveProfile(SaveProfileEvent event, Emitter<ProfileState> emit) async {
    ...
    result.fold(
      ...
      (_) {
        emit(ProfileSetupComplete(event.profileModel, event.isEdit));
      },
    );
  }
  ...
  Future<void> _onGetProfile(GetProfileEvent event, Emitter<ProfileState> emit) async {
    ...
    result.fold(
      (failure) => emit(ProfileStatusIncompleteState(_profileModel)),
      (profileModel) {
        _profileModel = profileModel;
        if (profileModel.isComplete) {
          emit(ProfileSetupComplete(profileModel,false));
        } else {
